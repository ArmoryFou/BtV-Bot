...
export default {
    ...
    async execute(message) {
        try {
            ...
            const reportField = embed.fields?.find(f => (f.name === 'Game Report' || f.name?.includes('Report')) && f.value.includes('kotobaweb.com'));
            ...
            if (member.roles.cache.has(config.roleId)) {
                ...
                await message.channel.send(`¡Buen intento <@${userId}>! Veo que ya posees el rol de este nivel, así que no se generará una nueva felicitación.`);
            ...
            const congratsChannel = await message.guild.channels.fetch(CONGRATS_CHANNEL_ID).catch(() => null);
            if (congratsChannel) {
